...
# Commercial language that must never appear in a reply to a support question.
_COMMERCIAL_IN_SUPPORT = [
    r"\bexpan(?:d|sion)\b", r"\bupgrade\b", r"\brenewal\b", r"\bnext agreement\b",
    r"\badditional workload\b", r"\banother workload\b", r"\bwhile we(?:'re| are) here\b",
    r"\bhave you considered\b", r"\bwould also benefit\b", r"\bcross[- ]?sell\b",
    r"\blicen[cs]e (?:extension|expansion)\b", r"\bnew contract\b",
]
...
def check_support_reply(text: str) -> GovernanceDecision:
    ...
    for pattern in _COMMERCIAL_IN_SUPPORT:
        if re.search(pattern, text or "", re.IGNORECASE):
            return GovernanceDecision(
                status=GOV_BLOCKED,
                text=decision.text,
                claim_level=decision.claim_level,
                violations=[*decision.violations, f"commercial_in_support:{pattern}"],
